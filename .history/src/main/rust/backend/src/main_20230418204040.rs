...
#[post("/sql_test", format = "json", data = "<sql_object>")]
pub async fn sql_test_event(db: &State<Database>, sql_object: Json<Event>) -> Result<status::Accepted<String>, rocket::http::Status> {
    match db.run(&sql_object.event_name).await {
        Ok(result) => Ok(status::Accepted(Some(format!("Result: {:?}", result)))),
        Err(_) => Err(rocket::http::Status::InternalServerError),
    }
}
